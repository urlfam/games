...
try {
  ...
  if (existingItems.length > 0) {
    ...
    // Cas 2: L'API retourne { data: [...] } ou { body: [...] }
      ...
      existingItems[0].json.data &&
      Array.isArray(existingItems[0].json.data)
    ) {
  ...
  console.log(
    '⚠️ Impossible de récupérer les jeux existants (nœud introuvable ?):',
    e.message,
  );
...
console.log(
  `📊 Analyse: ${scrapedGames.length} jeux scrapés vs ${existingGames.length} jeux existants`,
);
...
const newGames = scrapedGames.filter((scraped) => {
  ...
  const exists = existingGames.some((existing) => {
    ...
    return (
      existingTitle === scrapedTitle ||
      (scrapedUrl && existingUrl && existingUrl === scrapedUrl)
    );
  });
