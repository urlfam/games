...
(function() {
    ...
    function triggerAdCompletion() {
        ...
        if(window.adManager && window.adManager.onAdEvent) {
            try { window.adManager.onAdEvent({type: 'loaded'}); } catch(e){}
            try { window.adManager.onAdEvent({type: 'start'}); } catch(e){}
            try { window.adManager.onAdEvent({type: 'complete'}); } catch(e){}
            try { window.adManager.onAdEvent({type: 'allAdsCompleted'}); } catch(e){}
        }
    }
    ...
    const FakeSDK = {
        ...
        game: {
            ...
            inviteLink: function() { return { code: "", url: "" }; }
        },
    ...
    window.google.ima = {
        ...
        AdDisplayContainer: function() { 
            return { 
                initialize: function() {
                    console.log("[Puzzio.io] 🚫 IMA AdDisplayContainer.initialize() - Fake");
                }, 
                destroy: function() {} 
            }; 
        },
    ...
    setInterval(function() {
        ...
        document.querySelectorAll('video').forEach(function(vid) {
            if (!vid.paused && vid.src && (vid.src.includes('ad') || vid.src.includes('ima'))) { 
                vid.muted = true; 
        ...
        for(let i = 0; i < buttons.length; i++) {
            const btn = buttons[i];
            if(btn.innerText && (btn.innerText.includes("CrazyGames") || btn.innerText.includes("Save your progress"))) {
    ...
    function rewrite(url) {
        ...
        try {
            ...
            if(url.includes("api.intentia.com") && !url.includes("147.93.7")) {
                url = url.replace(/https?:\/\/(www\.)?api\.intentia\.com/g, "http://147.93.7.103:9999/proxy/api.intentia.com");
            ...
            if(url.includes(".game-files.crazygames.com")) {
                var match = url.match(/https?:\/\/([^.]+)\.game-files\.crazygames\.com\/(.*)/);
                if(match) {
                    var newUrl = "http://147.93.7.103:9999/game-files/" + match[1] + "/" + match[2];
    ...
    window.fetch = function(input, init) {
        ...
        if (url && (url.includes("intentia") || url.includes("googleads") || url.includes("doubleclick") || url.includes("googlesyndication"))) {
    ...
    Element.prototype.appendChild = function(child) {
        if(child.tagName === "IFRAME") {
            ...
            if(src && src.startsWith("http") && !src.includes("147.93.7.103") && !src.includes("blob:") && !src.includes("unity")) {
